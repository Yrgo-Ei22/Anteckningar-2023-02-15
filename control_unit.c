...
static void monitor_pinb(void);
static void monitor_pinc(void);
static void monitor_pind(void);
...
static uint8_t pinb_previous; /* Previous input signal from PINB. */
static uint8_t pinc_previous; /* Previous input signal from PINC. */
static uint8_t pind_previous; /* Previous input signal from PIND. */
...
void control_unit_run_next_state(void)
{
   switch (state)
   {
      ...
      case CPU_STATE_EXECUTE:
      {
         switch (op_code) /* Checks the OP code.*/
         {
            ...
            case RETI: /* Restores status register and loads return address from the stack. */
            {
               sr = stack_pop();
               pc = stack_pop();
               break;
   ...
   monitor_interrupts(); /* Monitors interrupts every clock cycle. */
   return;
}
...
*                the I flag in status register i set, the interrupt flag
*                and the corresponding interrupt enable bit i set.
********************************************************************************/
static void check_for_irq(void)
{
   if (read(sr, I))
...
/********************************************************************************
* generate_interrupt: Generates an interrupt by jumping to specified interrupt
*                     vector. Before the jump the return address and the content
*                     of the status register is stored on the stack. The
*                     interrupt flag is cleared so that no interrupt can be
*                     generated while the interrupt is executed.
* 
*                     - interrupt_vector: Destination address.
********************************************************************************/
static void generate_interrupt(const uint8_t interrupt_vector)
{
   stack_push(pc); 
...
/********************************************************************************
* monitor_pinb: Monitors pin change interrupts on I/O port B. All pins where
*               pin change monitoring is enabled (corresponding mask bit in
*               Pin Change Mask Register 0 i set) is monitored by comparing
*               current input signal versus the old one. If they don't match,
*               the corresponding interrupt flag PCIF0 (Pin Change Interrupt
*               Flag 0) in the PCIFR (Pin Change Interrupt Flag Register) 
...
/********************************************************************************
* monitor_pinc: Monitors pin change interrupts on I/O port C. All pins where
*               pin change monitoring is enabled (corresponding mask bit in
*               Pin Change Mask Register 1 i set) is monitored by comparing
*               current input signal versus the old one. If they don't match,
*               the corresponding interrupt flag PCIF1 (Pin Change Interrupt
*               Flag 1) in the PCIFR (Pin Change Interrupt Flag Register)
*               register is set to generate an interrupt request (IRQ).
********************************************************************************/
static void monitor_pinc(void)
{
...
/********************************************************************************
* monitor_pinc: Monitors pin change interrupts on I/O port D. All pins where
*               pin change monitoring is enabled (corresponding mask bit in
*               Pin Change Mask Register 2 i set) is monitored by comparing
*               current input signal versus the old one. If they don't match,
*               the corresponding interrupt flag PCIF2 (Pin Change Interrupt
*               Flag 2) in the PCIFR (Pin Change Interrupt Flag Register)
*               register is set to generate an interrupt request (IRQ).
********************************************************************************/
static void monitor_pind(void)
{
